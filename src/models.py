...
from modules import Encoder, LayerNorm, EncoderDrop, EncoderAug
...
class SASRecModelAugment(nn.Module):
    def __init__(self, args):
        super(SASRecModelAugment, self).__init__()
        self.item_embeddings = nn.Embedding(args.item_size, args.hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(args.max_seq_length, args.hidden_size)
        # self.item_encoder = Encoder(args)
        # self.item_encoder = EncoderDrop(args)
        self.item_encoder = EncoderAug(args)
        self.LayerNorm = LayerNorm(args.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(args.hidden_dropout_prob)
        self.args = args
    ...
    # model same as SASRec
    def transformer_encoder(self, input_ids, model_aug=True, isTrain=False):
        ...
        item_encoded_layers = self.item_encoder(sequence_emb,
                                                extended_attention_mask, output_all_encoded_layers=True,
                                                model_aug=model_aug, isTrain=isTrain)
    ...
    def most_similar(self, item, top_k=1, with_score=False):
        if self.model_name in ['ItemCF', 'ItemCF_IUF', 'Item2Vec', 'LightGCN']:
            ...
            if str(item) in self.similarity_model:
                ...
                print('cannot find %d in keys, random find one' %item)
                item_list = list(self.similarity_model.keys())
                random_items = random.sample(item_list, k=top_k)
                if with_score:
                    return list(map(lambda x: (int(x), 0.01), random_items)) # may also use the popular items?
