...
class ShortRandom(object):
    ...
    def __init__(self, tao=0.2, gamma=0.7, beta=0.2, \
                ...
                insert_threshold=14, insert_type='threshold'):
        self.augment_threshold = augment_threshold
        self.augment_type_for_short = augment_type_for_short
        Ins = ShortInsert(item_similarity_model, insert_num=insert_num,
                          max_insert_num_per_pos=max_insert_num_per_pos,
                          insert_threshold=insert_threshold, insert_type=insert_type)
        if self.augment_threshold == -1:
            self.data_augmentation_methods = [Crop(tao=tao), Mask(gamma=gamma), Reorder(beta=beta), Ins,
                                Substitute(item_similarity_model, substitute_rate=substitute_rate)]
            # print("Total augmentation numbers: ", len(self.data_augmentation_methods))
        elif self.augment_threshold > 0:
            # print("short sequence augment type:", self.augment_type_for_short)
            if self.augment_type_for_short == 'SI':
                self.short_seq_data_aug_methods = [Ins,
                                    Substitute(item_similarity_model, substitute_rate=substitute_rate)]
            elif self.augment_type_for_short == 'SIM':
                self.short_seq_data_aug_methods = [Ins,
                                    Substitute(item_similarity_model, substitute_rate=substitute_rate),
                ...
                self.short_seq_data_aug_methods = [Ins,
                                    Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                   Crop(tao=tao), Mask(gamma=gamma), Reorder(beta=gamma)]                
            self.long_seq_data_aug_methods = [Ins,
                                Crop(tao=tao), Mask(gamma=gamma), Reorder(beta=gamma),
                                Substitute(item_similarity_model, substitute_rate=substitute_rate)]
            # print("Augmentation methods for Long sequences:", len(self.long_seq_data_aug_methods))
            # print("Augmentation methods for short sequences:", len(self.short_seq_data_aug_methods))
...
class ShortInsert(object):
    """Insert similar items only for short sequences"""
    def __init__(self, item_similarity_model, insert_num=10, max_insert_num_per_pos=1,
            insert_threshold=14, insert_type='threshold'):
        self.threshold = insert_threshold
        if type(item_similarity_model) is list:
            self.item_sim_model_1 = item_similarity_model[0]
            self.item_sim_model_2 = item_similarity_model[1]
            self.ensemble = True
    ...
    def __call__(self, sequence):
        # make a deep copy to avoid original sequence be modified
        if self.insert_type == 'threshold':
            inserted_sequence = self.thresh_insert(sequence)
        elif self.insert_type == 'iteration':
            inserted_sequence = self.iter_insert(sequence)
        else:
            raise ValueError("Invalid insert type.")
        return inserted_sequence

    # wait: def a function to iteratively insert items
    def thresh_insert(self, sequence):
        cur_sequence = copy.deepcopy(sequence)
        # insert_nums = max(int(self.insert_num), 1)
        # cur_sequence = [i for i in copied_sequence] # initial seq.
        while len(cur_sequence) < self.threshold: # insert item until reaching the number of the threshold
            insert_idx = random.sample([i for i in range(len(cur_sequence))], k = 1)[0]
            top_k = self.max_insert_num_per_pos
            item = cur_sequence[insert_idx]
            if self.ensemble:
                top_k_one = self.item_sim_model_1.most_similar(item,
                                        top_k=top_k, with_score=True)
                top_k_two = self.item_sim_model_2.most_similar(item,
                                        top_k=top_k, with_score=True)
                correlated_items = _ensmeble_sim_models(top_k_one, top_k_two)
            else:
                correlated_items = self.item_similarity_model.most_similar(item,
                                        top_k=top_k)
            cur_sequence = cur_sequence[:insert_idx] + correlated_items + cur_sequence[insert_idx:]
        return cur_sequence
    
    def iter_insert(self, sequence):
        cur_sequence = copy.deepcopy(sequence)
        insert_nums = max(int(self.insert_num), 1)
        # inserted_sequence = []
        # cur_sequence = [i for i in copied_sequence] # initial seq.
        if len(cur_sequence) < self.threshold: # only insert if the sequence length < threshold 
            for _ in range(insert_nums):
                insert_idx = random.sample([i for i in range(len(cur_sequence))], k = 1)[0]
                top_k = self.max_insert_num_per_pos
                item = cur_sequence[insert_idx]
                if self.ensemble:
                    top_k_one = self.item_sim_model_1.most_similar(item,
                                            top_k=top_k, with_score=True)
                    # print('top_k_one:', top_k_one)
                    top_k_two = self.item_sim_model_2.most_similar(item,
                                            top_k=top_k, with_score=True)
                    # print('top_k_two:', top_k_two)
                    try: 
                        correlated_items = _ensmeble_sim_models(top_k_one, top_k_two)
                    except TypeError:
                        print('top_k_one:', top_k_one)
                        ...
                        print('current sequence:', cur_sequence)
                        print('items cannot find for:', item)
                else:
                    correlated_items = self.item_similarity_model.most_similar(item,
                                            top_k=top_k)
                cur_sequence = cur_sequence[:insert_idx] + correlated_items + cur_sequence[insert_idx:]
